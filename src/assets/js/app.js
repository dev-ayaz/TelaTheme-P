...
class App extends AppHelpers {
  ...
  moveInstagramSubtitle(){
    document.querySelectorAll('[class*="instagram-subtitle-"]').forEach(subtitle => {
      const position = subtitle.className.match(/instagram-subtitle-(\d+)/)?.[1];
      if(position){
        this.isElementLoaded(`#instagram-content-${position} .s-slider-block__title-right`).then(titleRight => {
          titleRight.appendChild(subtitle);
