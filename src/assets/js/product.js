...
class Product extends BasePage {
    ...
    initProductAccordion() {
        // Normalize initial state (remove any pre-rendered inline styles/actives)
        var allContentsInit = document.querySelectorAll('.product-accordion-content');
        for (var a = 0; a < allContentsInit.length; a++) {
            allContentsInit[a].style.maxHeight = '0';
            allContentsInit[a].classList.remove('active');
        }

        var accordionHeaders = document.querySelectorAll('.product-accordion-header');

        for (var i = 0; i < accordionHeaders.length; i++) {
            (function(index) {
                var header = accordionHeaders[index];
                var content = header.nextElementSibling;
                var icon = header.querySelector('.product-accordion-icon');

                function onTransitionEnd(e) {
                    if (e.target !== content) return;
                    if (content.classList.contains('active')) {
                        // keep content auto-sized when open
                        content.style.maxHeight = 'none';
                    }
                    content.removeEventListener('transitionend', onTransitionEnd);
                }

                function collapse(panel, panelIcon) {
                    // from auto to fixed height so transition works
                    ...
                    if (panelIcon) panelIcon.style.transform = 'rotate(0deg)';
                }

                function expand(panel, panelIcon) {
                    panel.removeEventListener('transitionend', onTransitionEnd);
                    panel.addEventListener('transitionend', onTransitionEnd);
                    panel.style.maxHeight = panel.scrollHeight + 'px';
                    panel.classList.add('active');
                    if (panelIcon) panelIcon.style.transform = 'rotate(180deg)';
                }

                header.addEventListener('click', function(e) {
                    e.preventDefault();
                    var wasOpen = content.classList.contains('active');

                    // Close ALL accordions reliably (use headers to pair icons/contents)
                    for (var h = 0; h < accordionHeaders.length; h++) {
                        var otherHeader = accordionHeaders[h];
                        var otherContent = otherHeader.nextElementSibling;
                        var otherIcon = otherHeader.querySelector('.product-accordion-icon');
                        if (otherContent.classList.contains('active')) {
                            collapse(otherContent, otherIcon);
                        }
                    }
